...
import { State, charactersAdapter, initialState } from './characters.reducer';
import * as CharactersSelectors from './characters.selectors';

describe('Characters Selectors', () => {
  const ERROR_MSG = 'No Error Available';
  const getCharactersId = (it) => it['id'];
  ...
  beforeEach(() => {
    state = {
      characters: charactersAdapter.addAll(
        [
  ...
  describe('Characters Selectors', () => {
    ...
    it('getSelected() should return the selected Entity', () => {
      const result = CharactersSelectors.getSelected(state);
